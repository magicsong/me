...
export async function getNoteById(id: number) {
  const result = await db.select({
    ...
    .from(notes)
    .leftJoin(notesTags, eq(notes.id, notesTags.note_id))
    .leftJoin(tags, eq(notesTags.tag_id, tags.id))
    ...
    if (tagNames && tagNames.length > 0) {
      ...
      for (const tagName of tagNames) {
        ...
        let tag = await tx.select()
          .from(tags)
          .where(and(eq(tags.name, tagName), eq(tags.user_id, userId)))
          .limit(1);
        ...
        if (tag.length === 0) {
          ...
          const [newTag] = await tx.insert(tags)
            .values({ name: tagName, user_id: userId, kind: 'note' })
            .returning();
          tagId = newTag.id;
        } else {
          tagId = tag[0].id;
        ...
        await tx.insert(notesTags)
          .values({ note_id: newNote.id, tag_id: tagId })
          .onConflictDoNothing();
      }
    }
